...
class ProductSeeder extends Seeder
{
    ...
    public function run()
    {
        // Les valeurs de base pour le produit, la taille, la capacité et la couleur
        $products = ['iPhone 14'];
        $sizes = ['6.1 pouces', '6.7 pouces'];
        $capacities = ['128 Go', '256 Go', '512 Go', '1024 Go'];
        $colors = ['Argent', 'Noir', 'Or', 'Violet'];

        // Parcourir chaque combinaison de produit, taille, capacité et couleur
            ...
            foreach ($sizes as $size) {
                foreach ($capacities as $capacity) {
                    foreach ($colors as $color) {
                        ...
                        // Créer une instance d'attribut pour la taille d'écran et l'attacher au SKU
                        $attribute = ProductAttribute::create(['product_id' => $product->id, 'name' => 'Taille d\'écran']);
                        $sku->attributes()->attach($attribute, ['value' => $size]);

                        // Créer une instance d'attribut pour la capacité et l'attacher au SKU
                        $attribute = ProductAttribute::create(['product_id' => $product->id, 'name' => 'Capacité']);
                        $sku->attributes()->attach($attribute, ['value' => $capacity]);

                        // Créer une instance d'attribut pour la couleur et l'attacher au SKU
                        $attribute = ProductAttribute::create(['product_id' => $product->id, 'name' => 'Couleur']);
                        $sku->attributes()->attach($attribute, ['value' => $color]);
                    }
                }
            }
        }
